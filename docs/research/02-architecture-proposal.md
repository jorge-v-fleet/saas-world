...
- **NPC engine** — stateful coworker personas, split into two layers so behavior stays deterministic:
  ...
  - **Determinism guards** — cache keyed by `(NPC state, context hash)` + log every call; replay reads the log and makes no model call, so eval reruns are reproducible. The Claude API has no `seed` and rejects `temperature`, so determinism is **record/replay, not sampling**. Grading is **state-based, not prose-based**, so parser phrasing never leaks into scores.
...
- **Stability guards (model-verification bar from the brief):**
  ...
  - *How stable*: narrow yes/no/field pulls (far more stable than quality ratings); cache + log + replay so a score reproduces exactly with no model call (the Claude API has no `seed`/`temperature` knobs — reproducibility is record/replay); **disagreement → flag for human review, never average**.
